...
Using the cleaned testing and training cross validation sets several models were fitted. An LDA model yielded accuracy of 0.7381. A CART model yielded an accuracy of 0.5011. Attempts to fit a boosting model caused the computer to crash. Finally, a random forest model was successfully fitted and demonstrated an accuracy of 0.9925 on the cross validation set. Due to its high accuracy the other models were discarded and the random forest model used for prediction on the final testing set. The predicted answers scored 100% upon submission to the Coursera site.
...
Training data set is 19,622 rows of 160 variables. Testing data set is 20 rows of 160 variables.
...
Accuracy is 0.7381 which is not very impressive. Perhaps if I have several different types of models I can create an ensemble prediction with better accuracy.
...
Accuracy for CART is only 0.5011, pretty poor, and unlikely to provide any help to an ensemble approach. I'm going to need some better accuracy.
...
It took a little over 3 hours to fit the model on my sad little Celeron-based laptop with 2GB of memory. But it was worth it. Accuracy on the cross validation set is 0.9925.
